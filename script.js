document.addEventListener('DOMContentLoaded', () => {
    ...
    function insertChar(char) {
        ...
        charsToShift.forEach(item => {
            ...
            delete paperData[item.key]; // 元の場所から削除
            paperData[`${item.x + GRID_SIZE},${item.y}`] = charToMove; // 新しい場所にコピー
        });
    ...
    // 【修正】後方削除（Backspace）ロジック
    function deleteCharBackward() {
        if (cursorPosition.x === 0) return;
        
        const deleteX = cursorPosition.x - GRID_SIZE;
        const currentY = cursorPosition.y;
        delete paperData[`${deleteX},${currentY}`];

        const charsToShift = Object.keys(paperData).map(key => ({ key, x: parseInt(key.split(',')[0]), y: parseInt(key.split(',')[1]) })).filter(item => item.y === currentY && item.x > deleteX).sort((a, b) => a.x - b.x);
